...
class VideoFrameExtractor
{
    ...
    /**
     * @var string|null Path to ffprobe executable
     */
    ...
    protected function detectFfprobePath()
    {
        ...
        // Try in same directory as ffmpeg first (common case)
        if (!empty($this->ffmpegPath)) {
            $sameDirPath = dirname($this->ffmpegPath) . DIRECTORY_SEPARATOR . 'ffprobe';
            Debug::log('Checking for ffprobe in same directory as ffmpeg: ' . $sameDirPath, __METHOD__);
            
            if (file_exists($sameDirPath) && is_executable($sameDirPath)) {
                Debug::log('Found ffprobe at: ' . $sameDirPath, __METHOD__);
                $this->ffprobePath = $sameDirPath;
                return $this->ffprobePath;
            }
        }
        ...
        foreach ($commonPaths as $path) {
            Debug::log('Checking for ffprobe at common path: ' . $path, __METHOD__);
            if (file_exists($path) && is_executable($path)) {
                Debug::log('Found ffprobe at: ' . $path, __METHOD__);
                $this->ffprobePath = $path;
                return $this->ffprobePath;
            }
        }
        
        // Try to detect using system commands (which, type, command -v)
        $detectionCommands = [
            'which ffprobe 2>/dev/null',
            'type -p ffprobe 2>/dev/null',
            'command -v ffprobe 2>/dev/null'
        ];
        
        foreach ($detectionCommands as $command) {
            Debug::log('Attempting to detect ffprobe using: ' . $command, __METHOD__);
            $output = [];
            $returnVar = null;
            ...
            if ($returnVar === 0 && !empty($output)) {
                ...
                if (file_exists($detectedPath) && is_executable($detectedPath)) {
                    Debug::log('Found ffprobe at: ' . $detectedPath, __METHOD__);
        ...
        if ($envPath) {
            ...
            foreach ($paths as $path) {
                $ffprobePath = rtrim($path, '/') . '/ffprobe';
                if (file_exists($ffprobePath) && is_executable($ffprobePath)) {
                    Debug::log('Found ffprobe in PATH: ' . $ffprobePath, __METHOD__);
        ...
        Debug::log('Failed to detect ffprobe executable', __METHOD__);
    ...
    protected function autoDetectPaths()
    {
        ...
        foreach ($possiblePaths as $path) {
            Debug::log('Checking for ffmpeg at common path: ' . $path, __METHOD__);
            if (file_exists($path) && is_executable($path)) {
                Debug::log('Found ffmpeg at: ' . $path, __METHOD__);
                return $path;
            }
        }
        ...
        $detectionCommands = [
            ...
            'type -p ffmpeg 2>/dev/null',
            'command -v ffmpeg 2>/dev/null'
        ];
        
        foreach ($detectionCommands as $command) {
            Debug::log('Attempting to detect ffmpeg using: ' . $command, __METHOD__);
            $output = [];
            $returnVar = null;
            @exec($command, $output, $returnVar);
            ...
            if ($returnVar === 0 && !empty($output)) {
                $detectedPath = trim($output[0]);
                if (file_exists($detectedPath) && is_executable($detectedPath)) {
                    Debug::log('Found ffmpeg at: ' . $detectedPath, __METHOD__);
                    return $detectedPath;
                }
            }
        }
        ...
        Debug::log('Failed to auto-detect ffmpeg executable', __METHOD__);
        return null;
    }
    ...
    public function getVideoDuration($videoPath)
    {
        ...
        if ($ffprobePath && is_executable($ffprobePath)) {
            Debug::log('Trying ffprobe method for duration using: ' . $ffprobePath, __METHOD__);
            $command = sprintf(
            ...
            Debug::log('No ffprobe executable found for duration detection', __METHOD__);
        }
